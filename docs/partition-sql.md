...
-- List all error_logs partitions
SELECT 
    schemaname,
    tablename,
    pg_size_pretty(pg_total_relation_size(schemaname||'.'||tablename)) AS size
...
FROM pg_tables
WHERE tablename LIKE 'audit_logs_%' OR tablename LIKE 'error_logs_%'
ORDER BY tablename;

-- 2. CREATE NEXT MONTH PARTITION (Update dates as needed)
-- Example: Creating for December 2025
CREATE TABLE audit_logs_2025_12 PARTITION OF audit_logs
    FOR VALUES FROM ('2025-12-01 00:00:00+00') TO ('2026-01-01 00:00:00+00');
...
-- 4. ARCHIVE OLD PARTITION (Update partition name as needed)
-- ALTER TABLE audit_logs DETACH PARTITION audit_logs_2024_01 CONCURRENTLY;
-- DROP TABLE IF EXISTS audit_logs_2024_01;
...
SELECT tablename FROM pg_tables WHERE tablename = 'audit_logs_2026_01';
